...
export function Logo({ variant = "default" }: Props) {
  ...
  return (
    <Link href="/" className="flex items-center gap-3 group" aria-label="Prodclas">
      <svg className="w-9 h-9 flex-shrink-0" viewBox="0 0 40 40" aria-hidden="true">
        <defs>
      ...
      <span className="flex flex-col leading-none">
        <span className={`font-serif font-medium text-[18px] tracking-[0.18em] ${textColor}`}>PRODCLAS</span>
        <span className={`font-serif italic text-[10px] mt-1 tracking-[0.04em] ${subColor}`}>
          Maison de Ceramică
        </span>
      </span>
    </Link>
